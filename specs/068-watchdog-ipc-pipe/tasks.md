...
- [ ] **T001**: Crear `app/ipc/__init__.py` y `app/ipc/watchdog_pipe.py` con `WatchdogIPCServer` y `WatchdogIPCClient`:
  ...
  - Fallback automático a Socket TCP Loopback `127.0.0.1:4029` si Named Pipe falla por permisos de Windows NT.
  ...
  - Hilo daemon con `threading.Event` para parada controlada.
...
- [ ] **T008**: Ejecución de suite de regresión completa: $\ge 1072$ tests PASS, 0 fallos, 0 regresiones.
