...
- [ ] **T003**: Extender `app/governance/chain_health.py` con `PredictiveChainEngine` y dataclass `PredictiveChainRisk`:
  - Evaluación de ventana deslizante de 12h: persistencia $\ge 90\%$ de fallos I2C.
...
- [ ] **T004**: Implementar discriminador de grupo eléctrico (`correlate_electrical_group`) para aislar perturbaciones en `elevator_1` y `elevator_2`.
...
- [ ] **T007**: Actualizar `app/config.example.json` con claves de configuración predictiva.

### Fase D: Pruebas Unitarias y Validación

- [ ] **T008**: Crear `tests/test_chain_predictive_rules.py` con $\ge 12$ pruebas:
...
- [ ] **T011**: Ejecución de suite completa de regresión: $\ge 1072$ tests PASS, 0 fallos, 0 regresiones.
