...
- Se añade el índice compuesto optimizado:
  ```sql
  ...
  ```
- Todas las consultas de diagnóstico histórico se ejecutan a través de `execute_readonly_with_retry` en el pool multi-lector.

### Componente 2: Reglas Matemáticas de Detección Predictiva
Implementadas en `app/governance/chain_health.py`:

1. **Regla 1 — Fallo Persistente de Sensor Térmico I2C**:
   - Condición: En una ventana móvil de 12 horas ($T_{\text{window}} = 43\,200\text{ s}$), $\ge 90\%$ de las muestras de la placa presentan `sensors_error_count > 0` con al menos una ubicación física idéntica (`faulty_sensor_locs`).
   - Disparo: Alerta preventiva a Telegram (deduplicada: máximo 1 alerta por cadena cada 24 horas):
     ...
     Minero: S19JPRO-24
     Cadena: 2 (Board 2)
     Diagnóstico: Sensor térmico I2C (loc 28) en fallo continuo por >12h.
...
3. **Regla 3 — Aislamiento Causal Eléctrico (Elevador vs Silicio)**:
   - Condición: Si $\ge 2$ mineros del mismo grupo eléctrico (`elevator_1` o `elevator_2`) sufren caídas de hashrate simultáneas dentro de un intervalo de 60 segundos:
...
3. **RF-03 (Tolerancia a Muestras Faltantes)**: Si un minero está apagado o en mantenimiento, el evaluador descarta la ventana sin generar falsas alarmas.
