...
10. **[EN OBSERVACIÓN - 85%] Spec 025 — Métricas de Prometheus y Paneles Grafana**: Módulo puro de instantáneas `app/metrics_snapshot.py` (T001-T005), exportador desacoplado `tools/metrics_exporter.py` (T007), stack Docker Compose aislado `docker-compose.observability.yml` (T008), datasource y 3 tableros Grafana (`fleet_overview`, `monitor_liveness`, `telegram_delivery`) (T009), 13 tests unitarios nuevos (404 tests PASS global, T002-T004, T011). Cableado al monitor (T006) y encendido live en contenedor (T012-T013) agendados para después del soak D+3.
